...
本项目基于 NJUST_AI 上游定制：**移除了**与账号、组织、市集浏览相关的上游云服务与 Marketplace 流程；**保留并扩展**了本地/自建服务对接能力——其中 **Cloud Agent** 通过可配置的 REST 服务端运行代理任务，是一套独立的远程推理协议，与"登录云账号、同步到商业云产品"无关。
...
│  │… │ │                                │  终端 / 调试 / 问题         │ │
│  └──┘ ├────────────────────────────────┴─────────────────────────────┤ │
│       │  NJUST_AI：对话 · 模式 · 工具审批 · 历史 · 设置（Webview）   │ │
│       └──────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────┘
...
文件变更触发扫描 → tree-sitter 解析分块 → 嵌入模型向量化 → 写入 Qdrant；自然语言查询经相同嵌入模型向量化后近邻检索，供 **codebase_search** 与 **NJUST_AIbaseSearch** 使用：
...
| Cloud Agent（REST） | 保留并增强 | 连接自建或内网服务；`njust-ai.cloudAgent.*` 配置服务端 URL、API Key、延期协议（deferred/start → resume 循环 + compile 反馈闭环）、远程 workspace 写操作等 |
...
- **ContextProxy**（`src/core/config/ContextProxy.ts`）：扩展内设置的单一可信来源，封装 `vscode.workspace.getConfiguration('njust-ai')` 的读写，与 VS Code `settings.json` 协同。
...
- **自定义模式**：用户可通过 `njust-ai.customModes` 设置或项目根目录 `.roomodes`（YAML）定义新模式或覆盖内置模式。
...
- **自动批准**（`src/core/auto-approval/`）：`njust-ai.autoApproval.*` 设置控制哪些工具可跳过审批，快捷键 `Ctrl+Alt+A`（macOS `Cmd+Alt+A`）快速切换策略。
...
- **Tool 接入**：Agent 通过 `codebase_search` 工具调用；VS Code 内置模型通过 `NJUST_AIbaseSearch` LM Tool。
...
- **命令安全**：`njust-ai.allowedCommands`（默认含 `git log`、`git diff`、`git show`）、`njust-ai.deniedCommands`、`commandExecutionTimeout`（默认 0 不限制）与 `commandTimeoutAllowlist`。
...
- **Chat Participant**（`src/chat/ChatParticipantHandler.ts`）：注册 `@njust-ai` 参与者，子命令 `code`、`architect`、`ask`、`debug`、`plan`，与 VS Code 内置 Chat UI 协作。支持 `disambiguation` 自动路由。
- **LM Tools**（`src/chat/registerLMTools.ts`）：注册 6 个 LM Tool——`NJUST_AI_readFile`、`NJUST_AI_editFile`、`NJUST_AI_executeCommand`、`NJUST_AI_searchFiles`、`NJUST_AI_listFiles`、`NJUST_AIbaseSearch`——供支持工具调用的内置模型使用。
...
- **ContextProxy**（`src/core/config/ContextProxy.ts`）：扩展内配置的单一可信来源，封装 `njust-ai.*` 全部设置项的读写。
...
- **Programmatic API**（`src/extension/api.ts`）：暴露 `NJUST_AIAPI` 接口——`startNewTask()`、`resumeTask()`、`sendMessage()`、`cancelCurrentTask()` 等。支持 `NJUST_AIAPI` 事件（taskStarted、taskCompleted、taskFailed 等）。
...
│   ├── chat/                           # VS Code Chat Participant（@njust-ai）与 LM Tools
│   ├── core/                           # 核心逻辑
│   │   ├── agent/                      # Agent 编排
...
│   ├── extension/                      # 对外 API（NJUST_AIAPI）、环境变量加载
...
│   ├── types/                          # @njust-ai/types — 共享类型定义
│   │   └── src/
│   │       ├── mode.ts                 # Mode 系统（DEFAULT_MODES、ModeConfig、Zod Schema）
│   │       ├── api.ts                  # NJUST_AIAPI / NJUST_AIIpcServer 接口
...
│   │       └── ...                     # 更多类型文件
│   ├── core/                           # @njust-ai/core — 平台无关核心逻辑
│   │   └── src/
│   │       ├── custom-tools/           # 自定义工具注册与序列化
│   │       ├── message-utils/          # 消息合并、指令序列合并、Token 统计
│   │       ├── task-history/           # 任务持久化历史管理
│   │       └── debug-log/              # 调试日志
│   ├── ipc/                            # @njust-ai/ipc — node-ipc 跨进程通信
│   ├── telemetry/                      # @njust-ai/telemetry — OpenTelemetry 集成（已简化）
│   ├── build/                          # @njust-ai/build — ESBuild 构建工具
│   ├── vscode-shim/                    # @njust-ai/vscode-shim — VS Code API 垫片
│   ├── evals/                          # @njust-ai/evals — 评估框架（Drizzle + PostgreSQL）
│   ├── config-eslint/                  # 共享 ESLint 配置
│   └── config-typescript/              # 共享 TypeScript 配置
├── apps/                               # 应用程序
│   ├── cli/                            # @njust-ai/cli — 命令行工具（njust-ai 二进制）
│   ├── vscode-e2e/                     # VS Code 端到端测试
│   ├── vscode-nightly/                 # Nightly 发布打包
│   ├── web-evals/                      # Web 评估界面（Next.js）
│   └── web-Njust-AI/                   # 公共网站（Next.js）
