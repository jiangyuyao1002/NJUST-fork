...
describe("QdrantVectorStore", () => {
	...
	describe("initialize", () => {
		it("should create a new collection if none exists and return true", async () => {
			...
			expect(mockQdrantClientInstance.createPayloadIndex).toHaveBeenCalledTimes(9)
		})
		...
		it("should log but not fail if payload index creation errors occur", async () => {
			...
			// Verify all payload index creations were attempted (9: type + 8 pathSegments)
			expect(mockQdrantClientInstance.createPayloadIndex).toHaveBeenCalledTimes(9)

			// Verify warnings were logged for each failed index (now 9)
	...
	describe("deleteCollection", () => {
		...
		it("should log and re-throw error when deletion fails", async () => {
			...
			expect(logger.error).toHaveBeenCalledWith(
				"QdrantVectorStore",
				`Failed to delete collection ${expectedCollectionName}:`,
				deleteError,
			)
	...
	describe("upsertPoints", () => {
		...
		it("should handle error scenarios when qdrantClient.upsert fails", async () => {
			...
			expect(mockQdrantClientInstance.upsert).toHaveBeenCalledTimes(1)
			expect(logger.error).toHaveBeenCalledWith("QdrantVectorStore", "Failed to upsert points:", upsertError)
	...
	describe("search", () => {
		...
		it("should handle error scenarios when qdrantClient.query fails", async () => {
			...
			expect(mockQdrantClientInstance.query).toHaveBeenCalledTimes(1)
			expect(logger.error).toHaveBeenCalledWith("QdrantVectorStore", "Failed to search points:", queryError)
