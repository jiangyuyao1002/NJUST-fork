...
export class SkillsManager {
	 ...
	 * - .njust_ai/skills can be a symlink to a directory containing skill subdirectories
	 * - .njust_ai/skills/[dirname] can be a symlink to a skill directory
	 */
	...
	async createSkill(
		...
		if (source === "global") {
			...
			baseDir = path.join(provider.cwd, NJUST_AI_CONFIG_DIR)
		}
...
`
	...
	> {
		...
		// - Global: .agents/skills first, then .njust_ai/skills (so .njust_ai wins)
		// - Project: .agents/skills first, then .njust_ai/skills (so .njust_ai wins)
		...
		// Global .njust_ai directories (higher priority than .agents)
		...
		// Project .njust_ai directories (highest priority)
		if (projectRooDir) {
	...
	private async setupFileWatchers(): Promise<void> {
		...
		const globalRooDir = getGlobalRooDirectory()
		const globalAgentsDir = getGlobalAgentsDirectory()
		const projectRooDir = path.join(provider.cwd, NJUST_AI_CONFIG_DIR)
		const projectAgentsDir = getProjectAgentsDirectoryForCwd(provider.cwd)

		// Watch global .njust_ai skills directory
		...
		// Watch project .njust_ai skills directory
