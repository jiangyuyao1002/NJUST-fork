...
describe("hidden directory exclusion", () => {
	...
	it("should allow explicit targeting of hidden directories", async () => {
		// Mock filesystem structure for explicit .njust_ai-memory targeting
		const mockReaddir = vi.fn()
		vi.mocked(fs.promises).readdir = mockReaddir

		// Mock .njust_ai-memory directory contents
		mockReaddir.mockResolvedValueOnce([
		...
		// Call listFiles explicitly targeting .njust_ai-memory directory
		const [result] = await listFiles("/test/.njust_ai-memory", true, 100)

		// When explicitly targeting a hidden directory, its subdirectories should be included
		const directories = result.filter((item) => item.endsWith("/"))

		const hasTasksDir = directories.some((dir) => dir.includes(".njust_ai-memory/tasks/") || dir.includes("tasks/"))
		const hasContextDir = directories.some(
			(dir) => dir.includes(".njust_ai-memory/context/") || dir.includes("context/"),
		)
	...
	it("should include top-level files when recursively listing a hidden directory that's also in DIRS_TO_IGNORE", async () => {
		// This test specifically addresses the bug where files at the root level of .njust_ai/temp
		...
		// Mock directory listing for .njust_ai/temp
		const mockReaddir = vi.fn()
		vi.mocked(fs.promises).readdir = mockReaddir
		mockReaddir.mockResolvedValueOnce([{ name: "22", isDirectory: () => true, isSymbolicLink: () => false }])

		// Call listFiles targeting .njust_ai/temp (which is both hidden and in DIRS_TO_IGNORE)
