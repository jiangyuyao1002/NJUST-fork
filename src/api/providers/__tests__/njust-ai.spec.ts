// npx vitest run api/providers/__tests__/njust-ai.spec.ts
...
describe("RooHandler", () => {
			...
			expect(mockCreate).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					headers: expect.objectContaining({
						"X-Njust-AI-App-Version": expect.any(String),
					}),
				}),
			)
		})
	...
	describe("completePrompt", () => {
		...
		it("should handle API errors", async () => {
			...
			await expect(handler.completePrompt("Test prompt")).rejects.toThrow(
				"NJUST_AI Cloud completion error: API Error",
			)
		})
	...
	describe("getModel", () => {
		...
		it("should handle any model ID since models are loaded dynamically", () => {
			...
			const testModelIds = ["xai/grok-code-fast-1", "njust-ai/sonic", "deepseek/deepseek-chat-v3.1"]
	...
	describe("temperature and model configuration", () => {
		...
		it("should use correct API endpoint", () => {
			// The base URL should be set to Njust-AI's API endpoint
			// We can't directly test the OpenAI client configuration, but we can verify the handler initializes
			handler = new RooHandler(mockOptions)
			expect(handler).toBeInstanceOf(RooHandler)
			// The handler should work with the Njust-AI API endpoint
		})
	})
