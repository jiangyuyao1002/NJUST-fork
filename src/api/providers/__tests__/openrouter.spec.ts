...
describe("OpenRouterHandler", () => {
	...
	describe("createMessage", () => {
		...
		it("throws when createMessage encounters an exception", async () => {
			const handler = new OpenRouterHandler(mockOptions)
		...
		it("throws on SDK exceptions with status 429", async () => {
			const handler = new OpenRouterHandler(mockOptions)
		...
		it("throws on SDK exceptions with 429 in message", async () => {
			const handler = new OpenRouterHandler(mockOptions)
		...
		it("throws on SDK exceptions containing 'rate limit'", async () => {
			const handler = new OpenRouterHandler(mockOptions)
		...
		it("throws on 429 rate limit errors from stream", async () => {
			const handler = new OpenRouterHandler(mockOptions)
			const mockStream = {
				async *[Symbol.asyncIterator]() {
		...
		it("yields tool_call_end events when finish_reason is tool_calls", async () => {
			...
			const handler = new OpenRouterHandler({
				...mockOptions,
				toolCallParser: mockToolCallParser,
			})
	...
	describe("completePrompt", () => {
		...
		it("throws on 429 rate limit errors from response", async () => {
			const handler = new OpenRouterHandler(mockOptions)
			const mockError = {
				error: {
