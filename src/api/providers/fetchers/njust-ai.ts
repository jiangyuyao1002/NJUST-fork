...
import { RooModelsResponseSchema } from "@njust-ai/core/providers"
 ...
 *
 * @param baseUrl The base URL of the NJUST_AI Cloud provider
 * @param apiKey The API key (session token) for the NJUST_AI Cloud provider
...
export async function getRooModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		try {
			...
			if (!parsed.success) {
				logger.error("RooModels", "Error fetching NJUST_AI Cloud models: Unexpected response format", data)
				logger.error("RooModels", "Validation errors:", parsed.error.format())
				throw new Error("Failed to fetch NJUST_AI Cloud models: Unexpected response format.")
			}
		...
		logger.error("RooModels", "Error fetching NJUST_AI Cloud models:", {
		...
		if (errorName === "AbortError") {
			throw new Error("Failed to fetch NJUST_AI Cloud models: Request timed out after 10 seconds.")
		}

		// Handle fetch errors
		if (errorMessage.includes("HTTP")) {
			throw new Error(`Failed to fetch NJUST_AI Cloud models: ${errorMessage}. Check base URL and API key.`)
		}

		// Handle network errors
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch NJUST_AI Cloud models: No response from server. Check NJUST_AI Cloud server status and base URL.",
			)
		}

		throw new Error(`Failed to fetch NJUST_AI Cloud models: ${errorMessage || "An unknown error occurred."}`)
	}
}
