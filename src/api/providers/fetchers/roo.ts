 ...
 *
 * @param baseUrl The base URL of the NJUST_AI_CJ Cloud provider
 * @param apiKey The API key (session token) for the NJUST_AI_CJ Cloud provider
...
export async function getRooModels(baseUrl: string, apiKey?: string): Promise<ModelRecord> {
	...
	try {
		...
		if (error instanceof TypeError) {
			throw new Error(
				"Failed to fetch NJUST_AI_CJ Cloud models: No response from server. Check NJUST_AI_CJ Cloud server status and base URL.",
			)
		}
