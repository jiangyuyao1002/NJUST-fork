...
describe("getRooModels", () => {
	const baseUrl = "https://localhost:8080/proxy"
	...
	it("should fetch and parse models successfully", async () => {
		...
		expect(mockFetch).toHaveBeenCalledWith(
			"https://localhost:8080/proxy/v1/models",
	...
	it("should handle HTTP errors", async () => {
		...
		await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch NJUST_AI_CJ Cloud models: HTTP 401: Unauthorized",
		)
	})
	...
	it("should handle timeout", async () => {
		...
		await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch NJUST_AI_CJ Cloud models: Request timed out",
		)
	})
	...
	it("should handle invalid response format", async () => {
		...
		await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch NJUST_AI_CJ Cloud models: Unexpected response format",
		)
	})
	...
	it("should normalize base URL correctly", async () => {
		...
		await getRooModels("https://localhost:8080/proxy/v1", apiKey)

		expect(mockFetch).toHaveBeenCalledWith("https://localhost:8080/proxy/v1/models", expect.any(Object))
	})
	...
	it("should handle network errors", async () => {
		...
		await expect(getRooModels(baseUrl, apiKey)).rejects.toThrow(
			"Failed to fetch NJUST_AI_CJ Cloud models: No response from server",
		)
	})
