...
import { getApiProtocol, type ImageGenerationApiMethod, TelemetryEventName } from "@njust-ai/types"
...
// Extend OpenAI's CompletionUsage to include Njust-AI specific fields
interface RooUsage extends OpenAI.CompletionUsage {
	cache_creation_input_tokens?: number
	cost?: number
}

// Add custom interface for Njust-AI params to support reasoning
...
export class RooHandler extends BaseOpenAiCompatibleProvider<string> {
	...
	constructor(options: ApiHandlerOptions) {
		...
		let baseURL = process.env.NJUST_AI_PROVIDER_URL ?? ""
	...
	protected override createStream(
		...
		// Get Njust-AI-specific reasoning parameters
	...
	protected override async *createMessageInner(
		...
		try {
			...
			const headers: Record<string, string> = {
				"X-Njust-AI-App-Version": Package.version,
			}

			if (metadata?.taskId) {
				headers["X-Njust-AI-Task-ID"] = metadata.taskId
			}
			...
			if (lastUsage) {
				...
				const apiProtocol = getApiProtocol("njust-ai", modelId)
	...
	override getModel() {
		...
		const models = getModelsFromCache("njust-ai") || {}
	...
	/**
	 * Generate an image using NJUST_AI Cloud's image generation API
	...
	async generateImage(
		...
		if (!sessionToken || sessionToken === "unauthenticated") {
			return {
				success: false,
				error: t("tools:generateImage.njust-ai.authRequired"),
			}
		}
