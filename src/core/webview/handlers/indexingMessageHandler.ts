...
async function handleSaveCodeIndexSettingsAtomic(
	...
	try {
		...
		if (s.codeIndexOpenAiKey !== undefined)
			await provider.contextProxy.storeSecret("codeIndexOpenAiKey", s.codeIndexOpenAiKey)
		if (s.codeIndexQdrantApiKey !== undefined)
			await provider.contextProxy.storeSecret("codeIndexQdrantApiKey", s.codeIndexQdrantApiKey)
		if (s.codebaseIndexOpenAiCompatibleApiKey !== undefined)
			await provider.contextProxy.storeSecret(
				"codebaseIndexOpenAiCompatibleApiKey",
				s.codebaseIndexOpenAiCompatibleApiKey,
			)
		if (s.codebaseIndexGeminiApiKey !== undefined)
			await provider.contextProxy.storeSecret("codebaseIndexGeminiApiKey", s.codebaseIndexGeminiApiKey)
		if (s.codebaseIndexMistralApiKey !== undefined)
			await provider.contextProxy.storeSecret("codebaseIndexMistralApiKey", s.codebaseIndexMistralApiKey)
		if (s.codebaseIndexVercelAiGatewayApiKey !== undefined)
			await provider.contextProxy.storeSecret(
				"codebaseIndexVercelAiGatewayApiKey",
				s.codebaseIndexVercelAiGatewayApiKey,
			)
		if (s.codebaseIndexOpenRouterApiKey !== undefined)
			await provider.contextProxy.storeSecret("codebaseIndexOpenRouterApiKey", s.codebaseIndexOpenRouterApiKey)
...
async function handleRequestCodeIndexSecretStatus(
	context: MessageHandlerContext,
		...
		if (mgr.isFeatureEnabled && mgr.isFeatureConfigured) {
			...
			if (mgr.state === "Standby" || mgr.state === "Error") {
				...
				if (!mgr.isInitialized) {
					await mgr.initialize(provider.contextProxy)
					if (mgr.state === "Standby" || mgr.state === "Error") void mgr.startIndexing()
				}
			}
		}
	} catch (error) {
		provider.log(`Error starting indexing: ${getErrorMessage(error)}`)
	}
}
...
function handleStopIndexing(context: MessageHandlerContext, _message: WebviewMessage): void {
	...
	try {
		...
		provider.log(`Error stopping indexing: ${getErrorMessage(error)}`)
	}
}
...
async function handleToggleWorkspaceIndexing(context: MessageHandlerContext, message: WebviewMessage): Promise<void> {
	...
	try {
		...
		if (enabled && mgr.isFeatureEnabled && mgr.isFeatureConfigured) {
			await mgr.initialize(provider.contextProxy)
			void mgr.startIndexing()
		} else if (!enabled) mgr.stopIndexing()
		void provider.postMessageToWebview({ type: "indexingStatusUpdate", values: mgr.getCurrentStatus() })
	} catch (error) {
		provider.log(`Error toggling workspace indexing: ${getErrorMessage(error)}`)
	}
}
...
async function handleSetAutoEnableDefault(context: MessageHandlerContext, message: WebviewMessage): Promise<void> {
	...
	try {
		...
		const all = provider.getAllCodeIndexManagers()
		...
		for (const m of all) {
			...
			else if (!was && now && m.isFeatureEnabled && m.isFeatureConfigured) {
		...
		provider.log(`Error setting auto-enable default: ${getErrorMessage(error)}`)
	}
}
...
async function handleClearIndexData(context: MessageHandlerContext, _message: WebviewMessage): Promise<void> {
	...
	try {
		...
		void provider.postMessageToWebview({
			type: "indexCleared",
			values: { success: false, error: getErrorMessage(error) },
		})
	}
}
