...
import type { ModelRecord, ExtensionMessage, Command } from "@njust-ai/types"
...
describe("webviewMessageHandler - requestRouterModels", () => {
	...
	it("handles individual provider failures gracefully", async () => {
		...
		mockGetModels
			...
			.mockResolvedValueOnce(mockModels) // njust-ai
	...
	it("handles Error objects and string errors correctly", async () => {
		...
		mockGetModels
			...
			.mockRejectedValueOnce(new Error("Njust-AI API error")) // njust-ai
			.mockRejectedValueOnce(new Error("LiteLLM connection failed")) // litellm
...
describe("webviewMessageHandler - requestOpenAiCodexRateLimits", () => {
	...
	it("posts values when authenticated", async () => {
		...
		mockFetchOpenAiCodexRateLimitInfo.mockResolvedValue({
			primary: { usedPercent: 10, resetsAt: 1700000000000, limit: 100, remaining: 90, reset: 1700000100000 },
			limit: 100,
			remaining: 90,
			reset: 1700000100000,
