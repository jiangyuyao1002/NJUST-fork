...
async function restoreModeBoundProfile(provider: ClineProvider, mode: string): Promise<void> {
	...
	try {
		...
		provider.log(
			`Failed to restore API configuration for mode '${mode}': ${getErrorMessage(
				error,
...
async function restoreTaskBoundProfile(provider: ClineProvider, apiConfigName: string): Promise<void> {
	...
	try {
		...
		provider.log(
			`Failed to restore API configuration '${apiConfigName}' for task: ${getErrorMessage(
				error,
...
export async function handleModeSwitchWithProvider(provider: ClineProvider, newMode: Mode): Promise<void> {
	...
	// so the persisted setting doesn't remain set while the mode is restricted.
	// The runtime guard in checkAutoApproval (ALWAYS_ALLOW_ALL_MODES check)
	// already prevents the bypass from being honored, but clearing the flag
	// keeps the persisted state consistent with the webview's setMode guard.
	const isModeAllowed = (ALWAYS_ALLOW_ALL_MODES as readonly string[]).includes(newMode)
	if (!isModeAllowed && provider.getGlobalState("alwaysAllowAll")) {
		await provider.contextProxy.setValue("alwaysAllowAll", false)
	}
...
async function persistTaskModeSwitch(provider: ClineProvider, newMode: Mode): Promise<void> {
	...
	try {
		...
		provider.log(`Failed to persist mode switch for task ${task.taskId}: ${getErrorMessage(error)}`)
		throw error
	}
}
...
async function activateModeSavedProfile(
	provider: ClineProvider,
	newMode: Mode,
...
export function getProviderProfileEntryWithProvider(
	provider: ClineProvider,
	name: string,
...
export async function upsertProviderProfileWithProvider(
	...
	try {
		...
		if (activate) {
			...
			await Promise.all([
				provider.settingsManager.setGlobalValue(
					"listApiConfigMeta",
			...
			await provider.settingsManager.setGlobalValue(
				"listApiConfigMeta",
				await provider.providerSettingsManager.listConfig(),
			)
		}
...
export async function deleteProviderProfileWithProvider(
	provider: ClineProvider,
	...
	if (profileToDelete.name === profileToActivate) {
		profileToActivate = getProviderProfileEntriesWithProvider(provider).find(
...
async function persistStickyProviderProfileToCurrentTask(
	...
	try {
		...
		provider.log(`Failed to persist provider profile switch for task ${task.taskId}: ${getErrorMessage(error)}`)
	}
}

async function persistCurrentTaskProfileName(
	provider: ClineProvider,
	taskId: string,
	apiConfigName: string,
): Promise<void> {
...
async function persistActivatedProfileModeBinding(
	provider: ClineProvider,
	id: string | undefined,
