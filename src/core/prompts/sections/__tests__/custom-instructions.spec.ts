...
describe("loadRuleFiles", () => {
	...
	it("should use .njust_ai/rules/ directory when it exists and has files", async () => {
		// Simulate .njust_ai/rules directory exists
		...
		statMock.mockImplementation((path) => {
			...
			if (
				normalizedPath.includes("/fake/path/.njust_ai/rules/file1.txt") ||
				normalizedPath.includes("/fake/path/.njust_ai/rules/file2.txt")
			) {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.njust_ai/rules/file1.txt") {
				return Promise.resolve("content of file1")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/file2.txt") {
	...
	it("should filter out cache files from .njust_ai/rules/ directory", async () => {
		// Simulate .njust_ai/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.njust_ai/rules/rule1.txt") {
				return Promise.resolve("rule 1 content")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/rule2.md") {
			...
			if (normalizedPath === "/fake/path/.njust_ai/rules/.DS_Store") {
				return Promise.resolve("DS_STORE_BINARY_CONTENT")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/Thumbs.db") {
				return Promise.resolve("THUMBS_DB_CONTENT")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/backup.bak") {
				return Promise.resolve("BACKUP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/cache.log") {
				return Promise.resolve("LOG_CONTENT")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/temp.tmp") {
				return Promise.resolve("TEMP_CONTENT")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/script.pyc") {
		...
		const expectedCacheFiles = [
			"/fake/path/.njust_ai/rules/.DS_Store",
			"/fake/path/.njust_ai/rules/Thumbs.db",
			"/fake/path/.njust_ai/rules/backup.bak",
			"/fake/path/.njust_ai/rules/cache.log",
			"/fake/path/.njust_ai/rules/temp.tmp",
			"/fake/path/.njust_ai/rules/script.pyc",
		]
	...
	it("should fall back to .roorules when .njust_ai/rules/ is empty", async () => {
		// Simulate .njust_ai/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
	...
	it("should read files from nested subdirectories in .njust_ai/rules/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			...
			},
			{
				...
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.njust_ai/rules/subdir",
			},
			{
				name: "nested2.txt",
				isFile: () => true,
				isSymbolicLink: () => false,
				isDirectory: () => false,
				parentPath: "/fake/path/.njust_ai/rules/subdir/subdir2",
			},
		] as any)
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.njust_ai/rules/root.txt") {
				return Promise.resolve("root file content")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/subdir/nested1.txt") {
				return Promise.resolve("nested file 1 content")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/subdir/subdir2/nested2.txt") {
		...
		const expectedRelativeNested2Path =
			process.platform === "win32"
				...
				: ".njust_ai/rules/subdir/subdir2/nested2.txt"
		...
		const expectedNested1Path2 =
			process.platform === "win32"
				...
				: "/fake/path/.njust_ai/rules/subdir/nested1.txt"
		const expectedNested2Path2 =
			process.platform === "win32"
				? "\\fake\\path\\.njust_ai\\rules\\subdir\\subdir2\\nested2.txt"
				: "/fake/path/.njust_ai/rules/subdir/subdir2/nested2.txt"
...
describe("addCustomInstructions", () => {
	...
	it("should use .njust_ai/rules-test-mode/ directory when it exists and has files", async () => {
		// Simulate .njust_ai/rules-test-mode directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
		...
		readdirMock.mockResolvedValueOnce([
			{
				...
				isFile: () => true,
				isSymbolicLink: () => false,
				parentPath: "/fake/path/.njust_ai/rules-test-mode",
			},
			{
				name: "rule2.txt",
				isFile: () => true,
		...
		statMock.mockImplementation((path) => {
			...
			const normalizedPath = path.toString().replace(/\\/g, "/")
			if (
				normalizedPath.includes("/fake/path/.njust_ai/rules-test-mode/rule1.txt") ||
				normalizedPath.includes("/fake/path/.njust_ai/rules-test-mode/rule2.txt")
			) {
				return Promise.resolve({
					isFile: vi.fn().mockReturnValue(true),
				}) as any
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.njust_ai/rules-test-mode/rule1.txt") {
				return Promise.resolve("mode specific rule 1")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules-test-mode/rule2.txt") {
		...
		const expectedAbsRule2Path =
			process.platform === "win32"
				? "\\fake\\path\\.njust_ai\\rules-test-mode\\rule2.txt"
				: "/fake/path/.njust_ai/rules-test-mode/rule2.txt"
	...
	it("should fall back to .roorules-test-mode when .njust_ai/rules-test-mode/ does not exist", async () => {
		// Simulate .njust_ai/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should fall back to .clinerules-test-mode when .njust_ai/rules-test-mode/ and .roorules-test-mode do not exist", async () => {
		// Simulate .njust_ai/rules-test-mode directory does not exist
		statMock.mockRejectedValueOnce({ code: "ENOENT" })
	...
	it("should correctly format content from directories when using .njust_ai/rules-test-mode/", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "rule1.txt", isFile: () => true, parentPath: "/fake/path/.njust_ai/rules-test-mode" },
		] as any)
		readFileMock.mockReset()
...
describe("Rules directory reading", () => {
	it.skipIf(process.platform === "win32")("should follow symbolic links in the rules directory", async () => {
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			// Handle both Unix and Windows path separators
			const normalizedPath = pathStr.replace(/\\/g, "/")
			if (normalizedPath === "/fake/path/.njust_ai/rules/regular.txt") {
				return Promise.resolve("regular file content")
			}
			if (normalizedPath === "/fake/path/.njust_ai/symlink-target.txt") {
				return Promise.resolve("symlink target content")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/symlink-target-dir/subdir_link.txt") {
				return Promise.resolve("regular file content under symlink target dir")
			}
			if (normalizedPath === "/fake/path/.njust_ai/nested-symlink-target.txt") {
		...
		const expectedRelativeRegularPath =
			process.platform === "win32" ? ".njust_ai\\rules\\regular.txt" : ".njust_ai/rules/regular.txt"
		const expectedRelativeSymlinkPath =
			process.platform === "win32" ? ".njust_ai\\symlink-target.txt" : ".njust_ai/symlink-target.txt"
		...
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.njust_ai/rules/link.txt")
		expect(readlinkMock).toHaveBeenCalledWith("/fake/path/.njust_ai/rules/link_dir")
	...
	it.skipIf(process.platform === "win32")("should correctly format multiple files from directory", async () => {
		...
		// Simulate listing files
		readdirMock.mockResolvedValueOnce([
			{ name: "file1.txt", isFile: () => true, parentPath: "/fake/path/.njust_ai/rules" },
			{ name: "file2.txt", isFile: () => true, parentPath: "/fake/path/.njust_ai/rules" },
		...
		statMock.mockImplementation((path) => {
			...
			expect([
				...
				"/fake/path/.njust_ai/rules/file3.txt",
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.njust_ai/rules/file3.txt") {
		...
		const expectedRelativeFile3Path =
			process.platform === "win32" ? ".njust_ai\\rules\\file3.txt" : ".njust_ai/rules/file3.txt"
	...
	it("should return files in alphabetical order by filename", async () => {
		...
		readdirMock.mockResolvedValueOnce([
			{ name: "zebra.txt", isFile: () => true, parentPath: "/fake/path/.njust_ai/rules" },
			{ name: "alpha.txt", isFile: () => true, parentPath: "/fake/path/.njust_ai/rules" },
		...
		readFileMock.mockImplementation((filePath: PathLike) => {
			...
			if (normalizedPath === "/fake/path/.njust_ai/rules/alpha.txt") {
				return Promise.resolve("alpha content")
			}
			if (normalizedPath === "/fake/path/.njust_ai/rules/Beta.txt") {
	...
	it("should sort symlinks by their symlink names, not target names", async () => {
		...
		// First call: check if .njust_ai/rules directory exists
		statMock.mockResolvedValueOnce({
			isDirectory: vi.fn().mockReturnValue(true),
		} as any)
