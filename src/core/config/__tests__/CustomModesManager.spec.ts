...
describe("CustomModesManager", () => {
	...
	describe("updateCustomMode", () => {
		it("should update mode in settings file while preserving .roomodes precedence", async () => {
			...
			;(fs.writeFile as Mock).mockImplementation(async function (
				path: string,
				content: string,
				_encoding?: string,
			) {
		...
		it("creates .roomodes file when adding project-specific mode", async () => {
			...
			;(fs.writeFile as Mock).mockImplementation(async function (path: string, content: string) {
				if (path === mockRoomodes) {
					roomodesContent = yaml.parse(content)
				}
				return Promise.resolve()
	...
	describe("File Operations", () => {
		...
		it("creates default config if file doesn't exist", async () => {
			...
			let firstCall = true
			;(fileExistsAtPath as Mock).mockImplementation(async function () {
				if (firstCall) {
					firstCall = false
					return false
				}
	...
	describe("deleteCustomMode", () => {
		it("deletes mode from settings file", async () => {
			...
			;(mockContext.globalState.update as Mock).mockImplementation(function (key: string, value: any) {
				if (key === "customModes") {
	...
	describe("updateModesInFile", () => {
		...
		describe("importModeWithRules", () => {
			...
			it("should prevent path traversal attacks in import", async () => {
				...
				;(fs.writeFile as Mock).mockImplementation(async function (path: string) {
				...
				// Verify that no files were written outside the .njust_ai directory
				const mockWorkspacePath = path.resolve("/mock/workspace")
				const writtenRuleFiles = writtenFiles.filter((p) => !p.includes(".roomodes"))
				writtenRuleFiles.forEach((filePath) => {
					const normalizedPath = path.normalize(filePath)
					const expectedBasePath = path.normalize(path.join(mockWorkspacePath, ".njust_ai"))
	...
	customModes:
			...
			it("should remove existing rules folder when importing mode without rules", async () => {
				...
				expect(fs.rm).toHaveBeenCalledWith(expect.stringContaining(path.join(".njust_ai", "rules-test-mode")), {
					recursive: true,
					force: true,
				})
	...
	describe("checkRulesDirectoryHasContent", () => {
		...
		it("should return true when rules directory has content files", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.njust_ai/rules-test-mode" },
			])
	...
	describe("exportModeWithRules", () => {
		...
		it("should successfully export mode with rules for a built-in mode customized in .roomodes", async () => {
			...
			;(fs.stat as Mock).mockResolvedValue({ isDirectory: () => true })
			;(fs.readdir as Mock).mockResolvedValue([
				{ name: "rule1.md", isFile: () => true, parentPath: "/mock/workspace/.njust_ai/rules-code" },
			])
		...
		it("should successfully export global mode with rules from global .njust_ai directory", async () => {
			// Mock a global mode
			const globalMode = {
			...
			;(fs.stat as Mock).mockImplementation(async function (path: string) {
				if (path.includes("rules-global-test-mode")) {
					return { isDirectory: () => true }
				}
				throw new Error("Directory not found")
			})
			;(fs.readdir as Mock).mockImplementation(async function (path: string) {
				if (path.includes("rules-global-test-mode")) {
