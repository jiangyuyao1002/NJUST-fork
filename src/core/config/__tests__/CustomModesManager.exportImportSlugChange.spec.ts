...
describe("CustomModesManager - Export/Import with Slug Changes", () => {
	...
	describe("Import with Slug Changes", () => {
		it("should place files in rules-{new-slug} folder when slug is changed", async () => {
			...
			;(fs.mkdir as Mock).mockImplementation(async function (path: string) {
			...
			expect(rule1Path).toContain(path.join(".njust_ai", "rules-new-slug-name", "rule1.md"))
			expect(rule2Path).toContain(path.join(".njust_ai", "rules-new-slug-name", "subfolder", "rule2.md"))
		...
		it("should handle mixed format paths correctly", async () => {
			...
			expect(oldFormatPath).toContain(path.join(".njust_ai", "rules-mixed-mode", "old-format.md"))
			expect(newFormatPath).toContain(path.join(".njust_ai", "rules-mixed-mode", "new-format.md"))
			expect(nestedPath).toContain(path.join(".njust_ai", "rules-mixed-mode", "nested", "file.md"))
		})
	})
	...
	describe("End-to-End Export/Import with Slug Change", () => {
		it("should successfully export and re-import with a different slug", async () => {
			...
			expect(ruleFilePath).toContain(path.join(".njust_ai", "rules-renamed-mode", "rule.md"))
