...
import { getGlobalRooDirectory } from "../../services/njust-ai-config"
...
export class CustomModesManager {
		...
		const handleSettingsChange = async () => {
			try {
				...
				try {
					...
					logger.error("CustomModesManager", "Error:", error)
	...
	public async updateCustomMode(slug: string, config: ModeConfig): Promise<void> {
		try {
			...
			logger.error("CustomModesManager", "Failed to update custom mode:", { slug, error: errorMessage })
			vscode.window.showErrorMessage(t("common:customModes.errors.updateFailed", { error: errorMessage }))
			throw error
		}
	}
	...
	private async deleteRulesFolder(slug: string, mode: ModeConfig): Promise<void> {
		try {
			...
			if (scope === "project") {
				...
				if (workspacePath) {
					rulesFolderPath = path.join(workspacePath, NJUST_AI_CONFIG_DIR, `rules-${slug}`)
				} else {
				...
				rulesFolderPath = path.join(homeDir, NJUST_AI_CONFIG_DIR, `rules-${slug}`)
			}
			...
			if (rulesFolderExists) {
				try {
					...
					logger.info("CustomModesManager", `Deleted rules folder for mode ${slug}: ${rulesFolderPath}`)
				} catch (error) {
					logger.error("CustomModesManager", `Failed to delete rules folder for mode ${slug}:`, error)
			...
			logger.error("CustomModesManager", `Error deleting rules folder for mode ${slug}:`, getErrorMessage(error))
		}
	}
	...
	/**
	 * Checks if a mode has associated rules files in the .njust_ai/rules-{slug}/ directory
	...
	public async checkRulesDirectoryHasContent(slug: string): Promise<boolean> {
		try {
			...
			if (isGlobalMode) {
				// For global modes, check in global .njust-ai directory
				const globalRooDir = getGlobalRooDirectory()
				modeRulesDir = path.join(globalRooDir, `rules-${slug}`)
			} else {
				// For project modes, check in workspace .njust-ai directory
				const workspacePath = getWorkspacePath()
				if (!workspacePath) {
					return false
				}
				modeRulesDir = path.join(workspacePath, NJUST_AI_CONFIG_DIR, `rules-${slug}`)
			}
	...
	public async exportModeWithRules(slug: string, customPrompts?: PromptComponent): Promise<ExportResult> {
		try {
			...
			let baseDir: string
			if (isGlobalMode) {
				// For global modes, use the global .njust-ai directory
			...
			// Check for .njust_ai/rules-{slug}/ directory (or rules-{slug}/ for global)
			...
			const errorMessage = getErrorMessage(error)
			logger.error("CustomModesManager", "Failed to export mode with rules:", { slug, error: errorMessage })
	...
	private async importRulesFiles(
		...
		if (source === "global") {
			...
			const workspacePath = getWorkspacePath()
			baseDir = path.join(workspacePath, NJUST_AI_CONFIG_DIR)
			rulesFolderPath = path.join(baseDir, `rules-${importMode.slug}`)
		}
		...
		for (const ruleFile of rulesFiles) {
			if (ruleFile.relativePath && ruleFile.content) {
				...
				if (normalizedRelativePath.includes("..") || path.isAbsolute(normalizedRelativePath)) {
					logger.error("CustomModesManager", `Invalid file path detected: ${ruleFile.relativePath}`)
				...
				if (!normalizedTargetPath.startsWith(expectedBasePath)) {
					logger.error("CustomModesManager", `Path traversal attempt detected: ${ruleFile.relativePath}`)
					continue // Skip this file but continue with others
				}
	...
	public async importModeWithRules(
		...
		try {
			...
			for (const importMode of importData.customModes) {
				...
				if (existingMode) {
					logger.info("CustomModesManager", `Overwriting existing mode: ${importMode.slug}`)
				}
			...
			const errorMessage = getErrorMessage(error)
			logger.error("CustomModesManager", "Failed to import mode with rules:", errorMessage)
			return { success: false, error: errorMessage }
		}
	}
