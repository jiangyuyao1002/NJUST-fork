...
describe("EditFileTool schema", () => {
	it("passes with valid input", () => {
		const result = editFileTool.inputSchema.safeParse({ file_path: "test.txt", old_string: "old", new_string: "new" })
		expect(result.success).toBe(true)
	})

	it("passes with optional expected_replacements", () => {
		const result = editFileTool.inputSchema.safeParse({ file_path: "test.txt", old_string: "old", new_string: "new", expected_replacements: 3 })
		expect(result.success).toBe(true)
	})
