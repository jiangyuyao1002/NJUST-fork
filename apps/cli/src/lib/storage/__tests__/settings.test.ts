...
const { getTestConfigDir } = vi.hoisted(() => {
	...
	const testConfigDir = path.join(os.tmpdir(), `njust-ai-cli-settings-test-${testRunId}`)
...
describe("Settings Storage", () => {
	...
	describe("loadSettings", () => {
		...
		it("should load saved settings", async () => {
			const settingsData = {
				onboardingProviderChoice: OnboardingProviderChoice.NjustAI,
				mode: "architect",
	...
	describe("resetOnboarding", () => {
		it("should reset onboarding provider choice", async () => {
			await saveSettings({ onboardingProviderChoice: OnboardingProviderChoice.NjustAI })
