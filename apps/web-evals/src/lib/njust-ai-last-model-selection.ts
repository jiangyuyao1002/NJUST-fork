...
export const NJUST_AI_LAST_MODEL_SELECTION_KEY = "evals-njust-ai-last-model-selection"
...
export function loadRooLastModelSelection(): string[] {
	...
	const parsed = modelIdListSchema.safeParse(tryParseJson(safeGetItem(NJUST_AI_LAST_MODEL_SELECTION_KEY)))
...
export function saveRooLastModelSelection(modelIds: string[]): void {
	...
	if (normalized.length === 0) {
		safeRemoveItem(NJUST_AI_LAST_MODEL_SELECTION_KEY)
		return
	}

	safeSetItem(NJUST_AI_LAST_MODEL_SELECTION_KEY, JSON.stringify(normalized))
}
