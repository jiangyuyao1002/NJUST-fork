...
1. **核心模块膨胀审查**：聚焦 `Task.ts`（5181 行）、`ClineProvider.ts`（3376 行）、`McpHub.ts`（2052 行，当前已缩减至 1475 行）三大核心文件的 SRP 违规、方法复杂度、状态管理与可测试性。
...
**综合架构质量评分为 2.8/5.0**（2026-04-15 初评），处于"功能完整但架构需要系统性改进"的阶段。好消息是，项目已开始部分模块化探索（如 `ToolExecutionOrchestrator`、`TaskLifecycle`、`TaskMetrics` 等），为后续重构奠定了基础。

> **2026-06-04 更新**：经过系统性重构，核心文件行数大幅下降（Task.ts 5181→1492 行，ClineProvider.ts 3376→1410 行，McpHub.ts 2052→1475 行），两条循环依赖已通过 `ITaskHost`/`IMcpHubClient` 接口打破并加入 CI 防回归，`ProviderRegistry` 取代了 25-branch switch，`packages/core` 扩展到 13 个子目录。新增：API 重试框架全量实现（`ApiRetryStrategy` 指数退避+真随机 jitter、`ApiErrorClassifier` 17 类 `ApiErrorKind`、`ApiRetryWrapper` 含流式重试），43 个 Provider 全量包装 `wrapApiHandler`；Anthropic SDK signal 传播修复（abort 时连接清理）；73 个浮动 Promise `.catch()` 处理器覆盖 7 个关键文件；`ContextProxy` dispose 守卫和 `setMaxListeners` 守卫。危险命令拦截实现四层防护体系。综合评分调整为 **3.7/5.0**——"架构健壮性持续提升，核心风险显著收敛"。剩余技术债集中在提示词系统解耦、Token 计数精度、审计日志完善等方面。

---

## 2. 架构质量评分卡

| 维度 | 初评 (2026-04) | 更新 (2026-06) | 说明 |
|------|:----:|:----:|------|
| **模块化** (Modularity) | 2.5/5 | **4.0/5** | Task.ts 从 5181→1492 行（拆出 20+ 个子模块），ClineProvider.ts 从 3376→1410 行（+4 辅助文件）。仍高于理想的 500 行/文件，但已从"上帝类"改善为"协调层+子模块"。 |
| **耦合度** (Coupling，分高=低耦合) | 2.0/5 | **4.0/5** | 两条循环依赖（webview↔task、mcp↔webview）已通过接口抽象打破，CI 防回归。API 重试框架（ApiRetryStrategy）已解耦 Provider 与重试逻辑。剩余分层违规（shared→core）待处理。 |
| **内聚性** (Cohesion) | 2.5/5 | **3.0/5** | Task 已拆分为生命周期/流处理/工具执行/消息管理等子模块，ClineProvider 拆出 Delegation/ModeSync/State。但进一步拆分仍需推进。 |
| **可扩展性** (Extensibility) | 3.0/5 | **4.5/5** | ProviderRegistry 取代 switch 工厂（新增 Provider 改 1 处文件），Tool 注册维持开闭原则（改 1 处文件）。API 重试策略模式（ApiRetryStrategy + ApiErrorClassifier）已实现，支持按提供商定制重试策略。 |
| **可维护性** (Maintainability) | 2.5/5 | **3.5/5** | 子模块拆分降低了单文件复杂度，接口定义（ITaskHost 等 9 个接口）改善了可理解性。重试/错误分类模式组织清晰（ApiRetryStrategy + ApiErrorClassifier + recoveryStrategyMap）。高复杂度方法和魔术数字问题仍存在。 |
| **可测试性** (Testability) | 3.0/5 | **3.5/5** | 拆出的子模块更易于单元测试，权限系统有 66+ 个回归测试。核心模块覆盖率阈值从 55%→65%，目标 80%。 |
| **综合评分** | **2.8/5** | **3.7/5** | **架构健壮性持续提升，核心风险显著收敛** |
...
> **2026-06-04 更新**：三大核心文件行数已大幅缩减。

三大核心文件合计曾超过 **10,609 行**，是项目最突出的架构问题。当前合计约 **4,377 行**（缩减 59%）。

- **Task.ts（5181→1492 行）**：已拆分为 20 个子模块（`TaskExecutor`、`TaskStreamConsumer`、`TaskLifecycleHandler`、`TaskRequestBuilder`、`TaskMessageManager` 等），总计约 9,800 行。通过 `ITaskHost` 等 9 个接口实现依赖倒置。Task.ts 本身作为协调层保留，仍需进一步缩减到 ≤500 行。
- **ClineProvider.ts（3376→1410 行）**：已拆出 `ClineProviderDelegation`（406 行）、`ClineProviderModeSync`（368 行）、`ClineProviderState`（69 行）、`ClineProviderProfiles`（18 行），总计 2,335 行。实现 5 个接口（WebviewViewProvider、TelemetryPropertiesProvider、TaskProviderLike、ITaskHost、IMcpHubClient）。
- **McpHub.ts（2052→1475 行）**：融合配置解析、三种传输协议建立、连接生命周期管理、资源同步、UI 回调等职责，单个连接方法约 190 行。（待后续 Phase 拆分）

### 3.2 模块间耦合

> **2026-06-03 更新**：两条循环依赖已通过接口抽象打破，CI 防回归守护。

**总体耦合度评级：7/10→5/10（改善）**

- ~~**循环依赖 1**（🔴 严重）：`core/webview` ↔ `core/task`~~ → ✅ **已修复**：Task 通过 `ITaskHost` 接口与 ClineProvider 通信，依赖方向为 webview→task 单向。
- ~~**循环依赖 2**（🔴 中高）：`services/mcp` ↔ `core/webview`~~ → ✅ **已修复**：`IMcpHubClient` + `IMcpStatusSink` 接口解耦。
- **分层违规**：`shared/modes.ts` 导入 `core/prompts`（破坏共享层独立性）；9 个 API Provider 文件导入 `core/assistant-message`；`utils` 层导入 `core`。（⚠️ 仍存在）
- **接口抽象评分：2.5/5→3.5/5**——`ITaskHost`（15+ 方法）、`IMcpHubClient`、`ITaskUINotifier`、`ITaskDiffViewProvider` 等 9 个接口已定义。但部分模块间仍通过直接导入通信。
- ~~**packages/core 内容不足**：仅 3 个子目录~~ → ✅ **已改善**：`packages/core/src/` 现有 13 个子目录，零 vscode 依赖。
...
| # | 问题描述 | 影响范围 | 涉及文件/模块 | 建议修复方案 | 状态 |
|---|---------|---------|-------------|------------|------|
| 1 | **工具权限 bypass 模式无安全警告** | 任何工具均可绕过权限检查，存在安全漏洞 | `src/core/tools/` 权限系统 | 为 bypass 模式添加显式安全警告和审计日志 | ⚠️ 部分修复：已有 `BypassWarningBanner` UI 警告和分类器链保护，待 audit.log 系统完善 |
| 2 | **~~命令执行缺少危险操作检查~~** | ~~用户可通过工具执行 `rm -rf` 等破坏性命令~~ | `execute_command` 工具 | ~~引入命令黑名单/白名单机制和确认提示~~ | ✅ **已修复**：`BashCommandAnalyzer`（6 类风险模式）+ `commandSafety` + `auto-approval/commands` 四层防护，forbidden 命令即使在 bypass 模式下也被阻止 |
| 3 | **API 层完全缺少指数退避重试** | 429/500 错误直接传播，用户体验差，服务中断风险 | `src/api/providers/` 全部 Provider | 实现通用重试框架，支持指数退避和可配置重试策略 | ⚠️ 待修复 |
| 4 | **API 层无 429 速率限制处理** | 高频调用时被限流后无法自动恢复 | `src/api/providers/`、`error-handler.ts` | 在重试框架中特别处理 429 状态码，根据 `Retry-After` 头延迟重试 | ⚠️ 待修复 |
| 5 | **~~核心循环依赖：webview ↔ task~~** | ~~两个 5000+/3300+ 行文件互相导入~~ | ~~`Task.ts`、`ClineProvider.ts`~~ | ~~引入事件总线或接口抽象~~ | ✅ **已修复**：通过 `ITaskHost` 接口实现依赖倒置，webview→task 单向依赖，CI 防回归守护 |
| 6 | **~~核心循环依赖：mcp ↔ webview~~** | ~~服务层反向依赖 UI 层~~ | ~~`McpHub.ts`、`McpServerManager.ts`、`ClineProvider.ts`~~ | ~~定义 `IMcpStatusSink` 接口~~ | ✅ **已修复**：`IMcpHubClient` + `IMcpStatusSink` 接口解耦，CI 防回归守护 |
| 7 | **Task.dispose 重复重置并发控制器** | 代码冗余，维护风险（本次提交引入） | `Task.ts` L2373-L2393 | 合并为单一 `try` 块 | ⚠️ 待确认 |

### P1 — 重要（影响可维护性和开发效率）

| # | 问题描述 | 影响范围 | 涉及文件/模块 | 建议修复方案 | 状态 |
|---|---------|---------|-------------|------------|------|
| 8 | **Token 计数全用 tiktoken，精度 ±15%** | 成本计算不准确，预算控制失效 | `base-provider.ts`、`countTokens.ts` | 为主要提供商实现 native token counting | ⚠️ 待修复 |
| 9 | **~~工厂函数 25+ switch 分支~~** | ~~新增提供商需修改核心文件 7+ 处~~ | ~~`src/api/index.ts`~~ | ~~改为提供商注册表模式~~ | ✅ **已修复**：`ProviderRegistry`（Map 工厂模式），新增 Provider 仅改 1 处文件 |
| 10 | **shared 层依赖 core 层** | 破坏共享层独立性，阻碍模块复用 | `shared/modes.ts` → `core/prompts` | 将被依赖的提示词常量下移到 shared 层 | ⚠️ 待修复 |
| 11 | **9 个 API Provider 依赖 core/assistant-message** | API 层与核心实现紧耦合 | 9 个 provider 文件 | 定义 `IToolCallParser` 接口，Provider 依赖接口 | ⚠️ 待修复 |
| 12 | **提示词系统嵌入大量业务逻辑** | cangjie-context（3134 行）混杂错误分析、import 解析等 | `cangjie-context.ts`、`custom-instructions.ts`、`multi-file-context.ts` | 分离为独立的服务（CangjieErrorAnalyzer、RuleFileManager、ImportContextResolver） | ⚠️ 待修复 |
| 13 | **参数验证不一致（3 种模式）** | 工具行为不可预测，维护困难 | 工具系统各工具实现 | 统一参数验证框架，使用 Schema 校验 | ⚠️ 待修复 |
| 14 | **编辑工具重叠** | `edit`/`edit_file`/`search_replace` 功能混淆 | 对应的 3 个工具文件 | 合并或明确区分使用场景，废弃冗余工具 | ⚠️ 待修复 |
| 15 | **SYSTEM_PROMPT() 13 个参数** | 调用复杂，扩展困难 | `system.ts` | 封装为 `PromptGenerationConfig` 对象 | ⚠️ 待修复 |
| 16 | **格式转换代码重复 1000+ 行** | 维护成本高，bug 修复需多处同步 | `src/api/transform/` 各 format 文件 | 抽取公共转换基类或工具函数 | ⚠️ 待修复 |

### P2 — 改进（提升代码质量和开发体验）

| # | 问题描述 | 影响范围 | 涉及文件/模块 | 建议修复方案 | 状态 |
|---|---------|---------|-------------|------------|------|
| 17 | **缺乏依赖注入框架** | 无法 mock、测试困难、环境切换困难 | 全项目 | 引入轻量级 DI 容器或工厂模式 | ⚠️ 待修复 |
| 18 | **~~packages/core 内容严重不足~~** | ~~平台无关核心逻辑仍在 src/~~ | ~~`packages/core/`（仅 3 子目录）~~ | ~~逐步迁移~~ | ✅ **已修复**：`packages/core/src/` 现有 13 个子目录（auto-approval、custom-tools、errors、feature-flags、message-queue、message-utils、prefetch、providers、security、shared、stream、task-history 等），零 vscode 依赖 |
| 19 | **提示词无版本管理** | 无法追踪/回滚提示词变化，无法 A/B 测试 | `src/core/prompts/` | 添加 Section 版本 ID 和变更日志 | ⚠️ 待修复 |
| 20 | **超时/重试配置硬编码** | 不同提供商/场景需不同配置 | `timeout-config.ts`、各 Provider | 支持按提供商和场景的细粒度超时配置 | ⚠️ 待修复 |
| 21 | **魔术数字散布** | 维护时需全文搜索，易遗漏 | 提示词系统多个文件 | 集中到 `prompts/config.ts` 导出命名常量 | ⚠️ 待修复 |
| 22 | **OpenAI-Native 单文件 1600 行** | 阅读和维护困难 | `openai-native.ts` | 拆分为 base/responses/tools/reasoning 四个文件 | ⚠️ 待修复 |
| 23 | **成本追踪无聚合分析** | 无法生成成本报告、无预算告警 | `globalCostTracker` | 添加会话级汇总、模型对比、异常检测 | ⚠️ 待修复 |
...
| 3 | ~~**实现 API 重试与错误分类框架**~~ | D.2 重试与错误分类框架 | ✅ **已修复**：ApiRetryStrategy（指数退避+真随机jitter）+ ApiErrorClassifier（17类ApiErrorKind）+ recoveryStrategyMap + ApiRetryWrapper（含流式重试），43个Provider全量包装 wrapApiHandler |
...
| 7 | ~~**实现 API 提供商注册表**~~ | D.1 ProviderRegistry 替代 switch | ✅ **已修复**：`ProviderRegistry`（Map 工厂模式）已取代 25-branch switch，新增 Provider 仅需改 1 处文件 |
